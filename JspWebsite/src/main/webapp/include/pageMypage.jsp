...
<body>
	<%
	MemberDTO mep = (MemberDTO) session.getAttribute("loginMember");
	Page p1 = new Page();
	...
	switch(md){
	case "mypost":
		tp1 = p1.getMypostPage(mep);
		break;
	case "myreply":
		tp1 = p1.getMyreplyPage(mep);
		break;		
	}
	...
	%>
	<a href="mypage.jsp?page=1&mode=<%=md%>"
	...
	<a style="font-weight: bolder;"
		href="mypage.jsp?page=<%=i%>&mode=<%=md%>"><%=i%></a>
	<%
	} else {
	%>
	<a href="mypage.jsp?page=<%=i%>&mode=<%=md%>"><%=i%></a>
	<%
	}
	}
	...
	%>
	<a href="mypage.jsp?page=<%=(pb * Page.PAGE_BLOCK + 1)%>&mode=<%=md%>"
	...
	%>
	<a href="mypage.jsp?page=<%=tp1%>&mode=<%=md%>"
