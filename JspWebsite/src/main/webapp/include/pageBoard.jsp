...
<body>
	...
	if (cg1 == null) {//페이징만 볼거면무시해도됨
		cg1 = "general";
	}
	Page p1 = new Page();
	String where = "where category = '" + cg1 + "'";
	int tp1 = p1.getTotalPage(where); //총 페이지수
	...
	%>
	<a href="board.jsp?page=1&category=<%=cg1 %>"
	...
	<a style="font-weight: bolder;"
		href="board.jsp?page=<%=i%>&category=<%=cg1%>"><%=i%></a>
	<%
	} else {
	%>
	<a href="board.jsp?page=<%=i%>&category=<%=cg1%>"><%=i%></a>
	<%
	}
	}
	...
	%>
	<a href="board.jsp?page=<%=(pb * Page.PAGE_BLOCK + 1)%>&category=<%=cg1 %>"
	...
	%>
	<a href="board.jsp?page=<%=tp1 %>&category=<%=cg1 %>"
