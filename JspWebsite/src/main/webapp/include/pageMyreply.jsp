...
<body>
	...
	if (cPage1 > 1) {
	%>
	<a href="/board/myreply?page=1&mode=<%=md%>"
	...
	%>
	<a
		href="/board/myreply?page=<%=((pb - 1) * Page.PAGE_BLOCK)%>&mode=<%=md%>"
		style="font-weight: bolder; font-size: 0.8em;">이전</a>
	<%
	} else if (cPage1 > 1) {
	%>
	<a href="/board/myreply?page=<%=(cPage1 - 1)%>&mode=<%=md%>"
		style="font-weight: bolder; font-size: 0.8em;">이전</a>
	<%
	}
	} else {
	...
	<a style="font-weight: bolder;"
		href="/board/myreply?page=<%=i%>&mode=<%=md%>"><%=i%></a>
	<%
	} else {
	%>
	<a href="/board/myreply?page=<%=i%>&mode=<%=md%>"><%=i%></a>
	<%
	}
	}
	...
	%>
	<a href="/board/myreply?page=<%=(pb * Page.PAGE_BLOCK + 1)%>&mode=<%=md%>"
	...
	if (cPage1 < tp1) {
	%>
	<a href="/board/myreply?page=<%=tp1%>&mode=<%=md%>"
